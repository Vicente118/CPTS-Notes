...
1. Figure out the exact organization name from the IMAP/POP3 service and submit it as the answer. -> nmap
	- InlaneFreight Ltd
2.  What is the FQDN that the IMAP and POP3 servers are assigned to? -> nmap
	- dev.inlanefreight.htb
3.  Enumerate the IMAP service and submit the flag as the answer. (Format: HTB{...})
	```shell
	> openssl s_client -connect 10.129.105.94:imaps
	HTB{roncfbw7iszerd7shni7jr2343zhrj}
	```
4. What is the customized version of the POP3 server?
	```shell
	> openssl s_client -connect 10.129.105.94:pop3s
	InFreight POP3 v9.188
	```

5. What is the admin email address?
```shell
> openssl s_client -quiet -connect 10.129.105.94:imaps
? LOGIN robin robin
? LIST "" *
